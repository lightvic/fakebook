...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $target_dir = __DIR__ . "/../../public/img_pages_groups/";
    $full_name = $_FILES["upload_picture"]["name"];
    ...
    if ($_FILES["upload_picture"]["size"] > 5971520) {
    ...
    if ($uploadOk === 0) {
        ...
        if (move_uploaded_file($_FILES["upload_picture"]["tmp_name"], $target_file)) {
            $maRequete = $pdo->prepare(
                "UPDATE `groups` SET `picture` = :group_picture WHERE `group_id` = :groupId"
            );
            $maRequete->execute([
                ":group_picture" => $full_name,
                ":groupId" => $group_id
            ]);
            $_SESSION['group']['picture'] = $full_name;
